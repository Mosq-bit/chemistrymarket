...
class ChemistryMarketAPI {
  ...
  initDemoProducts() {
    ...
    const demoProductsData = [
      ...
      {
        id: 302017,
        name: "Гидроксид натрия каустический",
        cas: "1310-73-2",
        formula: "NaOH",
        price: "120",
        category: "alkalis",
      ...
      {
        id: 302020,
        name: "Соляная кислота реактивная",
        cas: "7647-01-0",
        formula: "HCl",
        price: "85",
        category: "acids",
      ...
      {
        id: 302021,
        name: "Азотная кислота концентрированная",
        cas: "7697-37-2",
        formula: "HNO₃",
        price: "110",
        category: "acids",
      ...
      {
        id: 302028,
        name: "Этиленгликоль антифризный",
        cas: "107-21-1",
        formula: "C₂H₆O₂",
        price: "115",
      ...
      {
        id: 302032,
        name: "Карбонат кальция осажденный",
        cas: "471-34-1",
        formula: "CaCO₃",
        price: "55",
        category: "additives",
    ...
    console.log(
      "Демо-данные инициализированы:",
      this.demoProductsMap.size,
      "товаров",
    );
  }
  ...
  getProductDescription(productName) {
    const descriptions = {
      кислот:
        "Высококачественная техническая кислота промышленного назначения. Используется в химической промышленности, металлургии, производстве удобрений. Соответствует ГОСТ и международным стандартам.",
      гидроксид:
        "Каустическая сода высокой чистоты. Применяется в производстве бумаги, мыла, моющих средств, в нефтепереработке. Гарантированное качество и стабильность поставок.",
      ацетон:
        "Ацетон высшей степени очистки. Используется как растворитель в лакокрасочной промышленности, производстве пластмасс, фармацевтике. Низкое содержание примесей.",
      "диоксид титана":
        "Рутильная форма диоксида титана с высокими кроющими свойствами. Применяется в производстве красок, лаков, пластмасс, бумаги. Отличная диспергируемость.",
      спирт:
        "Высококачественный растворитель органического происхождения. Используется в фармацевтической, парфюмерной промышленности, в качестве антисептика. Различные степени очистки.",
    };
  ...
  getPackagingForProduct(productName) {
    if (productName.includes("кислот") || productName.includes("спирт")) {
      ...
      productName.includes("порошок") ||
      productName.includes("оксид")
    ) {
      return "Мешок 25 кг";
    } else {
      const packagings = [
  ...
  async fetchProductDetail(productId) {
    ...
    console.warn(
      `Товар с ID ${productId} не найден в демо-данных, создаем новый`,
    );
  ...
  async fetchProducts(page = 1) {
    ...
    if (this.apiAvailable) {
      try {
        ...
        console.warn(
          "Ошибка API при запросе товаров, используем демо-данные:",
          error.message,
        );
        this.apiAvailable = false;
      }
  ...
  transformApiResponse(apiData, page) {
    ...
    let filtered = apiData.results.filter((p) => {
      ...
      if (
        ...
        return false;
      if (this.filters.cas && !p.cas_number?.includes(this.filters.cas))
        return false;
      return true;
    });
  ...
  getDemoData(page) {
    ...
    const demoProductIds = Array.from(this.demoProductsMap.keys()).sort(
      (a, b) => a - b,
    );
    ...
    return {
      ...
      next:
        page < Math.ceil(this.demoProductsMap.size / this.perPage)
          ? `?page=${page + 1}`
          : null,
...
class ProductManager {
  ...
  async updateApiStatus() {
    ...
    try {
      ...
      statusElement.innerHTML =
        '<i class="fas fa-sync fa-spin"></i> Проверка подключения к API...';
      ...
      if (isApiAvailable) {
        ...
        statusElement.innerHTML =
          '<i class="fas fa-check-circle"></i> Подключено к реальному API (данные с сервера)';
        console.log("✅ Используется реальный API");
      } else {
        statusElement.className = "api-status offline";
        statusElement.innerHTML =
          '<i class="fas fa-database"></i> Используются демо-данные (оффлайн режим)';
      ...
      statusElement.className = "api-status offline";
      statusElement.innerHTML =
        '<i class="fas fa-database"></i> Используются демо-данные';
    }
  }
  ...
  showDetailModal(productDetail) {
      ...
      ? `
            <div style="margin-top: 20px;">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 15px;">
                    ${Object.entries(productDetail.specifications)
                      .map(
                        ([key, value]) => `
    ...
    const filesHTML =
      productDetail.files && productDetail.files.length > 0
        ? `
            <div style="margin-top: 20px;">
                <h4 style="color: var(--primary-dark); margin-bottom: 15px; border-bottom: 2px solid var(--accent-teal); padding-bottom: 5px;">
                    Документация
  ...
  async requestQuote(productId) {
    ...
    try {
      const productDetail = await this.api.fetchProductDetail(productId);
      console.log(
        `Получены данные товара: ${productDetail.name} (ID: ${productDetail.id})`,
      );
                ...
                <form id="quoteRequestForm" class="quote-request-form">
                    <div class="form-group">
                        ...
                        <input type="text" name="quantity" class="form-control" 
                               placeholder="Например: 10 " required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="unit" class="form-control"
                                        ...
                                        <option value selected disabled>Выберите
                                            единицу измерения</option>
                                        <option value="кг">Килограммы
                                            (кг)</option>
                                            <option value="л">Граммы (г)</option>
                                        <option value="л">Литры (л)</option>
                                        <option value="шт">Миллилитры (мл)</option>
                                        <option value="м">Тонны (тн)</option>
                                    </select>
                    </div>
    ...
    form.addEventListener("submit", async (e) => {
      ...
      if (phoneElement) {
        ...
        if (!phoneRegex.test(data.phone.trim())) {
          this.showModalError(
            phoneElement,
            "Пожалуйста, укажите телефон в формате (999) 123-45-67",
          );
          isValid = false;
        }
      }
      ...
      if (isValid) {
        const quoteRequests = JSON.parse(
  ...
  showModalError(element, message) {
    ...
    errorDiv.style.cssText =
      "color: #dc2626; font-size: 0.85rem; margin-top: 5px; font-weight: 500;";
    ...
    quoteForm.addEventListener("submit", async (e) => {
      ...
      fields.forEach((field) => {
        ...
        if (!element || !element.value.trim()) {
          this.showFormError(
            element,
            `Пожалуйста, заполните поле "${field.name}"`,
          );
          isValid = false;
        }
      });
...
window.requestQuote = (productId) =>
  window.productManager?.requestQuote(productId);
